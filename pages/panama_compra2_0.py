...
PC2_HIDDEN_DEFAULT_COLUMNS = {
    "ficha_detectada_original",
    "score_ficha_v2",
    "confianza_ficha_v2",
    "evidencia_ficha_v2",
    "motivo_ficha_v2",
    "tiene_ct_v2",
    "registro_sanitario_v2",
    "enlace_ficha_minsa_v2",
    "clase_ficha_v2",
}
...
@st.cache_data(ttl=600)
def _pc2_detection_metadata_df(file_id: str) -> pd.DataFrame:
    fid = _normalize_drive_file_id(file_id)
    if not fid:
        return pd.DataFrame()
    try:
        return load_drive_excel(fid)
    except Exception:
        return pd.DataFrame()


def _pc2_apply_ficha_detection(df: pd.DataFrame, *, metadata_df: pd.DataFrame | None = None) -> pd.DataFrame:
    if not isinstance(df, pd.DataFrame) or df.empty:
        return df

    available_norm_cols = {_normalize_column_key(col) for col in df.columns}
    has_text_fields = any(
        key in available_norm_cols
        for key in {
            "titulo",
            "descripcion",
            "item 1",
    ...
    ) or any(_normalize_column_key(col).startswith("item") for col in df.columns)

    if not has_text_fields:
        return df

    try:
        return apply_detection_v2_to_dataframe(df, metadata_df=metadata_df)
    except Exception:
        return df
...
def _pc2_load_fichas_db_meta(
    ...
    if not ficha_num_col:
        return pd.DataFrame()
    candidate_cols = [
        ficha_num_col,
        _resolve_column_by_alias(fichas_columns, ["nombre generico", "nombre ficha", "nombre"]),
        _resolve_column_by_alias(fichas_columns, ["tiene ct", "con ct", "ct", "criterio tecnico", "criterio"]),
        _resolve_column_by_alias(fichas_columns, ["registro sanitario", "registro_sanitario", "reg sanitario"]),
        _resolve_column_by_alias(
            fichas_columns,
            [
                "enlace_ficha_tecnica",
                "enlace ficha tecnica",
        ...
        _resolve_column_by_alias(fichas_columns, ["clase", "categoria", "clasificacion", "tipo"]),
    ]
    selected_cols = tuple(dict.fromkeys([col for col in candidate_cols if col]))
    if not selected_cols:
        return pd.DataFrame()
    try:
        return _load_table_subset(
            backend,
            db_url,
            db_path_str,
            fichas_table,
...
@st.cache_data(ttl=300)
def _build_prospeccion_rir_dataframe_pc2(
    backend: str,
    db_url: str,
    db_path_str: str,
    actos_table: str,
    ...
    enlace_col = _resolve_column_by_alias(actos_columns, ["enlace", "url", "link"])
    ganador_col = _resolve_column_by_alias(
        actos_columns,
        ["razon_social", "razon social", "adjudicatario", "proveedor ganador"],
    )
    titulo_col = _resolve_column_by_alias(actos_columns, ["titulo", "título"])
    desc_col = _resolve_column_by_alias(actos_columns, ["descripcion", "descripción"])
    ficha_original_col = _resolve_column_by_alias(
        actos_columns,
        ["ficha_detectada", "ficha detectada", "ficha", "numero de ficha"],
    )
    proponente_cols = [
    ...
    item_cols = [
        col
        for col in actos_columns
        if _normalize_column_key(col).startswith("item")
    ]

    selected_cols: list[str] = []
    for candidate in [titulo_col, desc_col, ficha_original_col, precio_col, enlace_col, ganador_col]:
        if candidate and candidate not in selected_cols:
            selected_cols.append(candidate)
    for candidate in item_cols + proponente_cols:
        if candidate and candidate not in selected_cols:
            selected_cols.append(candidate)
    if not selected_cols:
        return pd.DataFrame()
    ...
    if meta_source_mode in {"auto", "drive"} and fichas_drive_file_id:
        metadata_df = _pc2_detection_metadata_df(fichas_drive_file_id)
    if metadata_df.empty and meta_source_mode in {"auto", "db"} and fichas_table:
        metadata_df = _pc2_load_fichas_db_meta(backend, db_url, db_path_str, fichas_table)

    actos_df = _pc2_apply_ficha_detection(actos_df, metadata_df=metadata_df)
    ficha_col = _resolve_column_by_alias(
        actos_df.columns,
        ["ficha_detectada", "ficha detectada", "ficha", "numero de ficha"],
    )
    ...
    detected_mask = (
        ...
        .str.lower()
        .ne("no detectada")
    ) & actos_df[ficha_col].fillna("").astype(str).str.strip().ne("")
    actos_df = actos_df[detected_mask].copy()
    if actos_df.empty:
        return pd.DataFrame()
    ...
    for _, row in actos_df.iterrows():
        fichas = _extract_ficha_tokens(row.get(ficha_col, ""))
        if not fichas:
            continue
        unique_ficha = fichas[0] if len(fichas) == 1 else ""
        precio_ref = _parse_money_value(row.get(precio_col, "")) if precio_col else None
        enlace = _clean_text(row.get(enlace_col, "")) if enlace_col else ""
        ganador = _clean_text(row.get(ganador_col, "")) if ganador_col else ""
        proponentes_row = {
            _clean_text(row.get(col, ""))
            for col in proponente_cols
        ...
        for ficha_token in fichas:
            ...
            if proponentes_row:
                state["proponentes"].update(proponentes_row)
            if str(row.get("tiene_ct_v2", "")).strip().lower() == "si":
                state["ct_yes"] = True
            if str(row.get("registro_sanitario_v2", "")).strip().lower() == "si":
                state["rs_yes"] = True
            if not state["enlace_minsa"]:
                state["enlace_minsa"] = _normalize_minsa_link(row.get("enlace_ficha_minsa_v2", ""))
            if not state["clase"]:
                state["clase"] = _clean_text(row.get("clase_ficha_v2", ""))
            try:
                state["scores"].append(float(row.get("score_ficha_v2", 0.0) or 0.0))
            except Exception:
                pass
            conf = _clean_text(row.get("confianza_ficha_v2", ""))
            if conf:
                state["confidence"][conf] += 1
    ...
    for ficha_token, state in stats.items():
        ...
        top1_text, top1_count = _winner_text(1)
        top2_text, top2_count = _winner_text(2)
        top1_unique = int(state["ganadores_unicos"].get(ganadores_sorted[0][0], 0)) if ganadores_sorted else 0
        top2_unique = int(state["ganadores_unicos"].get(ganadores_sorted[1][0], 0)) if len(ganadores_sorted) >= 2 else 0
        top1_pct_present = (top1_count / actos_presentes * 100.0) if actos_presentes else 0.0
        top1_pct_unique = (top1_unique / actos_unicos * 100.0) if actos_unicos else 0.0
        top2_pct_present = (top2_count / actos_presentes * 100.0) if actos_presentes else 0.0
        top2_pct_unique = (top2_unique / actos_unicos * 100.0) if actos_unicos else 0.0

        links_sorted = sorted(state["links"], key=lambda pair: pair[0], reverse=True)
        links_unique: list[str] = []
        seen_links: set[str] = set()
        for _, raw_link in links_sorted:
            link = _clean_text(raw_link)
        ...
        confidence_top = "Alta"
        if state["confidence"]:
            confidence_top = state["confidence"].most_common(1)[0][0]
        avg_score = (
            sum(float(v) for v in state["scores"]) / len(state["scores"])
            if state["scores"]
            else 0.0
        )
        ...
        rows.append(
            {
                "Ficha #": ficha_token,
                "Nombre ficha": f"Ficha {ficha_token}",
                "Actos con ficha": actos_presentes,
                "Actos ficha unica": actos_unicos,
                "Monto total (ficha unica)": monto_unicos,
                "Tiene criterio tecnico": "Si" if state["ct_yes"] else "No",
                "Registro sanitario": "Si" if state["rs_yes"] else "No",
                "Enlace ficha MINSA": _normalize_minsa_link(state["enlace_minsa"]),
                "Proponentes distintos": proponentes_distintos,
                "Top 1 ganador": top1_text,
                "% Top 1 (total, unica)": f"{top1_pct_present:.1f}%, {top1_pct_unique:.1f}%",
                "Top 2 ganador": top2_text,
                "% Top 2 (total, unica)": f"{top2_pct_present:.1f}%, {top2_pct_unique:.1f}%",
                "Confianza ficha v2": confidence_top,
                "Score promedio v2": round(avg_score, 2),
                "__meta_found__": "Si" if state["enlace_minsa"] or state["ct_yes"] or state["rs_yes"] else "No",
                "__actos_links__": "\n".join(links_unique),
            }
        )
    ...
    *,
    ...
    try:
        with st.spinner("Construyendo Prospeccion RIR..."):
            df = _build_prospeccion_rir_dataframe_pc2(
                backend=backend,
                db_url=db_url,
...
st.title("📋 Visualizador de Actos Panamá Compra 2.0")
st.caption(
    "Versión paralela de validación. Recalcula la detección de fichas sobre las mismas fuentes, "
    "con evidencia + scoring + abstención, sin alterar Panamá Compra 1.0."
)
...
def render_df(
    ...
    with st.expander("🔎 Filtros", expanded=True):
        ...
        if item_cols_sorted:
            non_item_cols = [
                c for c in displayable_columns if c not in item_cols_sorted and c not in PC2_HIDDEN_DEFAULT_COLUMNS
            ]
            ...
            if cols_key not in st.session_state:
                st.session_state[cols_key] = [c for c in displayable_columns if c not in PC2_HIDDEN_DEFAULT_COLUMNS]
    ...
    if "score_ficha_v2" in display_df.columns:
        display_df["score_ficha_v2"] = pd.to_numeric(display_df["score_ficha_v2"], errors="coerce")
        col_cfg["score_ficha_v2"] = st.column_config.NumberColumn("score_ficha_v2", format="%.2f")
    ...
    if "enlace_ficha_minsa_v2" in display_df.columns:
        col_cfg["enlace_ficha_minsa_v2"] = st.column_config.LinkColumn(
            label="MINSA v2",
            display_text="MINSA",
            help="Abrir ficha sugerida en MINSA",
        )
...
for tab, category_name in zip(category_tabs, ordered_categories):
    with tab:
        ...
        df = _pc2_apply_ficha_detection(load_df(sheet_name))
        if category_name == "Criterios Tecnicos RIR":
            _render_ct_rir_manager(df, key_prefix=f"{selector_key}_ct_rir")
        if df.empty:
            st.info("Sin datos en esta pestana.")
