...
def _inject_modern_style() -> None:
    """Inyecta estilos globales para un look & feel mǭs pulido sin tocar la l��gica."""
    st.markdown(
        """
<style>
...
:root {
  ...
  --pc-border-strong: rgba(45,212,191,0.55);
  --pc-text: #e8edf5;
  --pc-muted: #9fb3c8;
  --pc-accent: #2dd4bf;
  --pc-accent-2: #38bdf8;
...
.stApp {
  background: radial-gradient(130% 120% at 18% 10%, #1c3d7138 0%, transparent 50%),
              radial-gradient(120% 120% at 80% 0%, #0ea5e933 0%, transparent 45%),
              linear-gradient(120deg, #0b1224 0%, #0b1530 48%, #0f1f3f 100%);
  color: var(--pc-text);
...
.pc-hero {
  ...
  padding: 18px;
  margin-top: 0.6rem;
  background: linear-gradient(140deg, rgba(45,212,191,0.14), rgba(14,165,233,0.12));
  border: 1px solid var(--pc-border);
  border-radius: 18px;
...
.pc-status-card__title {
  font-weight: 600;
  color: var(--pc-text);
}

.pc-status-card__detail {
  color: var(--pc-muted);
  margin-top: 6px;
  font-size: 0.9rem;
  line-height: 1.35;
...
def _render_page_header() -> list[dict[str, Any]]:
    """Encabezado visual con chips de estado de las fuentes base."""
    statuses = _collect_core_source_statuses()
    ready = sum(1 for item in statuses if item.get("ok"))
    total = len(statuses)
    title = f"{ready}/{total} fuentes listas" if total else "Fuentes no configuradas"

    chips = []
    for item in statuses or [{"name": "Fuentes", "ok": False, "detail": "A��ade rutas base para iniciar."}]:
        name = html.escape(str(item.get("name", "Fuente")))
        detail = html.escape(str(item.get("detail", "")))
        cls = "pc-chip" + ("" if item.get("ok") else " pc-chip--warn")
        chips.append(
            f"""
<div class="{cls}">
  <div class="pc-chip__dot"></div>
  <div>
    <div class="pc-chip__label">{name}</div>
    <div class="pc-chip__detail">{detail}</div>
  </div>
</div>
"""
        )
...
<div class="pc-hero">
  <div class="pc-hero__main">
    <div class="pc-hero__eyebrow">PanamaCompra · tablero</div>
    <div class="pc-hero__title">Visualizador de actos con acabado moderno</div>
    <p class="pc-hero__body">
      Monitorea adjudicaciones, fichas y estatus de bots en un layout mǭs limpio y contrastado, sin tocar los datos.
    </p>
    <div class="pc-badges">
      <span class="pc-pill">Filtros r��pidos y edici��n directa</span>
      <span class="pc-pill">Tarjetas tonales para estados</span>
      <span class="pc-pill">Tabs amplias y botones con realce</span>
    </div>
  </div>
  <div class="pc-hero__status">
    <div class="pc-hero__status-title">{title}</div>
    <div class="pc-hero__chips">
      {''.join(chips)}
    </div>
  </div>
</div>
...
def _render_data_health_summary(statuses: list[dict[str, Any]] | None = None) -> bool:
    statuses = statuses or _collect_data_health_statuses()
    all_ok = all(item.get("ok") for item in statuses) if statuses else True
    cards = []
    for item in statuses:
        cls = "pc-status-card ok" if item.get("ok") else "pc-status-card warn"
        cards.append(
            f"""
<div class="{cls}">
  <div class="pc-status-card__title">{html.escape(str(item.get('name', 'Fuente')))}</div>
  <div class="pc-status-card__detail">{html.escape(str(item.get('detail', '')))}</div>
</div>
"""
        )
    ...
    with st.expander("Estado de datos base", expanded=not all_ok):
        st.markdown(
            f"""<div class="pc-status-grid">{''.join(cards)}</div>""",
            unsafe_allow_html=True,
        )
    return all_ok
...
_inject_modern_style()
source_statuses = _render_page_header()
_render_data_health_summary(source_statuses)
