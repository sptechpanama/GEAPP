...
def build_estado_resultados(
    ...
    *,
    ...
    ingresos_operativos = float(pd.to_numeric(_safe_series(ingresos_resultado.loc[nature_res.eq("Operativo")], COL_MONTO, 0.0), errors="coerce").fillna(0.0).sum())
    ingresos_financieros_base = float(pd.to_numeric(_safe_series(ingresos_resultado.loc[nature_res.eq("Financiero")], COL_MONTO, 0.0), errors="coerce").fillna(0.0).sum())
    ingresos_no_operativos = float(pd.to_numeric(_safe_series(ingresos_resultado.loc[nature_res.eq("No operativo")], COL_MONTO, 0.0), errors="coerce").fillna(0.0).sum())
    ...
    costos_directos = float(pd.to_numeric(_safe_series(gas_resultado.loc[subclas_result.eq("Costo directo")], COL_MONTO, 0.0), errors="coerce").fillna(0.0).sum())
    gastos_operativos = float(pd.to_numeric(_safe_series(gas_resultado.loc[subclas_result.isin(["Administrativo fijo", "Operativo variable", "Comercial / ventas", "No operativo"])], COL_MONTO, 0.0), errors="coerce").fillna(0.0).sum())
    gastos_financieros_base = float(pd.to_numeric(_safe_series(gas_resultado.loc[subclas_result.eq("Financiero")], COL_MONTO, 0.0), errors="coerce").fillna(0.0).sum())
    impuestos_mask = subclas_result.eq("Impuestos") | _safe_series(gas_resultado, COL_CATEGORIA, "").eq("Impuestos")
    impuestos = float(
        pd.to_numeric(_safe_series(gas_resultado.loc[impuestos_mask], COL_MONTO, 0.0), errors="coerce")
        .fillna(0.0)
        .sum()
    )
